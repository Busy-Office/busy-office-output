...
- [ ] Archive profile = **PDF/A-2b**, veraPDF joins the corpus gates (ADR-006); pdf-direct gains embedded TTF + XMP + OutputIntent if it stays in play — DoD: veraPDF clean on every corpus artifact in CI — **blocked on GATE-VERAPDF-INSTALL** (`docs/HUMAN-GATES-LOG.md`, open); pdf-direct is not in scope for this stage's renderer per ADR-001 (Typst owns pagination for this document type), so that half is moot
...
Note: this exit gate as literally written doesn't name veraPDF/PDF-A-2b, but `docs/STANDARDS.md` Tier 2 requires it for any archive-format compliance claim — that task is still open, blocked on `GATE-VERAPDF-INSTALL` (`docs/HUMAN-GATES-LOG.md`). Stage 2's own written exit-gate text is satisfied; the PDF/A-2b task above it is not, and stays open pending the gate.
