...
- [x] **GAP-22** Reproduce returns bytes only, no re-delivery (the row holds no recipients; email needs a message template rendered from a payload the runtime doesn't hold — GAP-10). Low priority — DoD: `reproduce` gains `deliverTo?` for object-store/fs channels only, or a decision on email re-delivery without a payload *(2026-08-30: `reproduce` gained `deliverTo?: { channel: 'object-store'; recipients: string[] }` on `create-output.ts`'s `ReproduceInput` — `object-store` only, checked against the real `ChannelSender` implementations (`fs`/`filesystem` is the channel-agnostic default sender, not a distinct channel of its own standing); `email` stays excluded per GAP-10. Enqueues through the SAME `DeliveryQueue.enqueue` composition.ts already uses — no new delivery path, no synchronous send. `admitReprint`/`stampReprint` unchanged, redelivery additive on top of the existing audit stamp. `npm run verify` green 82/82 files, 521/521 tests, incl. new `reproduce-redeliver.test.ts` (2/2))*
- [x] **[HUMAN]** **GAP-23** Visible REPRINT watermark — closed by the frozen expression grammar (text values must be envelope-rooted paths, no literals; contracts are `additionalProperties: false`). Needs a `reprint` grammar root (grammar doc first, ADR-005-style review) or nothing. Never a modification of archived bytes — DoD: maintainer decides yes/no *(2026-08-30: ruled NO — no visible watermark; the existing metadata-only `reprint_log` stamp is final, not interim. No grammar/parser/renderer change. docs/GAP-REGISTER.md GAP-23)*
...
- [x] **GAP-20** Publishing v2 does not retire v1 of the same variant (tie-breaks by registration order) — low priority, named by the Stage 5 task-1 ruling as "log, don't solve" — DoD: either publish auto-retires the superseded same-variant version (maintainer policy) or task 4's screen makes the superseded version's live status visible for explicit retire *(2026-08-29: ruled directly in chat, GATE-S5-RULINGS item 5 — no auto-retire-on-publish; first-registered-wins tie-break stands, retire must be explicit. docs/GAP-REGISTER.md GAP-20)*
...
## Stage 5 — Governance  `~3–4 weeks · CLOSED 2026-08-29`
...
- [x] Overview (failures-first home) + Settings (four flat groups) — DoD: overview is nearly empty when all green *(2026-08-29, 3a78ef9 — corpus-qa re-derived: all-green body byte-EQUAL (not containment) to `<h1>Overview</h1><p>Nothing needs attention.</p>`; one-of-each-kind fixture → correct order and exactly one link per row; stranded-outbox row never double-listed under both the outbox and render-failed groups; `setStatus('in_progress')` proven to bump `updated_at`; `listDocuments({state, search, offset})` composes; NO-SECRET sweep of 8 planted credential/PII strings absent from every body AND header across 9+ routes; type-level credential lock caught live — planting `accessKeyId` into `ConsoleFacts` fails `tsc` with the lock's own error; nav byte-identical across 6 pages; depth crawl ≤ 2 from every route. 446/446. `STRANDED_AFTER_MS` = 5 min ratified 2026-08-29 (GATE-S5-RULINGS item 4) — kept as-is. Still-open maintainer items, untouched by this ruling: `awsCredentialsConfigured()` reads two AWS env vars beyond the three ratified path vars (boolean only, provably never leaves index.ts); bare-server Settings 404s; stuck-scan bounded at 200 rows pre-filter)*

### Exit gate — `/gate-check 5`
A template change cannot reach PRD without an approval record — the test
attempts it and fails.

**MET** *(2026-08-29 — corpus-qa re-derived all three proofs at 3a78ef9:
pure table `transitions.test.ts` 9/9 (48/48 full file), through-the-port
`stage5-exit-gate.test.ts` 3/3, through-the-screen
`stage5-exit-gate-screen.test.ts` 7/7. All five Stage 5 tasks now carry
an independent gate-check verdict in git history. **Stage 5 CLOSED
2026-08-29**: the maintainer ruled directly in chat — both of ADR-007's
addenda (OutputPort v1 surface; v1.1 reprint verbs) Accepted as drafted,
satisfying "no stage closes with its ADRs open" (`GATE-S5-CLOSE`
closed); GAP-13's ratified exception text read broadly, covering this
close and Stage 6's start, not just the building that already happened.
The bundled `GATE-S5-RULINGS` (seven small items) closed the same
session — all seven stated defaults ratified as policy. See
docs/HUMAN-GATES-LOG.md and docs/SESSION-LOG.md 2026-08-29.)*
...
- [x] Variant exercise: country/company/customer overrides via inheritance, zero template forking — DoD: resolver + render tests *(2026-08-29: GAP-27 closed — arb-chair scoped the merge to `fieldGrid.fields[].label`/`table.columns[].key`/`totals.rows[].label` only, child-wins, `section`/`document`/`header`/`footer` whole-subtree (docs/GAP-REGISTER.md). `packages/schema/src/variant/merge.ts` (`mergeTemplateContent`, 13 tests in merge.test.ts including resolveTemplate+resolveParentChain composition) is wired into the REAL path: `DocumentTypeRegistry.templateContent()` (packages/runtime/src/registration/document-type-registry.ts) now calls `resolveParentChain` + `mergeTemplateContent` instead of a flat map lookup — the exact function `composeRenderArchiveAndEnqueue` (composition.ts) calls for every resolution, not an orphaned function. Registration also gained parentId-existence + cycle validation (atomic, same file). 9 registry-level tests (document-type-registry-variant-inheritance.test.ts) register a base + 3 overrides (country/company/customer) as tiny fieldGrid/totals fragments and prove per-override: overridden field differs, everything else byte-identical to base, no override is a whole document tree. 5 render tests (test/corpus/purchase-order/014-variant-inheritance.test.ts) prove the same 3 overrides through a real `typst compile` of the SAME `purchaseOrderTemplate` every other corpus case uses — overridden label present in the override's PDF text, absent from the base's, specific untouched fields (PO number, buyer/vendor names, other totals labels) confirmed present via targeted `pdftotext` substring checks in both renders (not a full structural diff — the registry-level tests above are the ones proving true structural equality). `npm run verify` 80 files/508 tests pass (up from 77/481); corpus-qa independently gate-checked (wiring traced to the real composition call site, merge direction hand-verified, ruling scope confirmed respected) — one run during its check hit a flake in an unrelated pre-existing test (document-detail-reproduce.test.ts, 400-vs-404), not reproduced across three subsequent full-suite runs, see docs/SESSION-LOG.md.)*
