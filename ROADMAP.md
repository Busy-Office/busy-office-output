...
- [ ] **pdf-direct second renderer** (ADR-002 Accepted, maintainer's explicit choice to keep it scheduled rather than reserved-not-adopted): `packages/render-pdf-direct` behind the same `Renderer` interface, **PDF/A-2b via embedded TTF + XMP + OutputIntent, veraPDF-clean in the corpus gate** — DoD: corpus green with pdf-direct rendering the simple/single-page cases, veraPDF passes on every pdf-direct artifact; routing rule (which templates it serves, bounded by ADR-001 — multi-page/carry-forward/non-Latin stay Typst) decided in this task. A pdf-direct that renders fast but fails veraPDF is not done (ADR-006)
