...
- [ ] Bursting through fan-out at target volume; second renderer (ADR-002, default pdf-lib) lands here if needed — DoD: gate below *(gap register GAP-03: run the bursting measurement Typst-only through the real pipeline first — ADR-002's "pending real-hardware numbers" now exist; Claude measures, human decides ADR-002)*
...
- [ ] **GAP-11** `serve()` still mints pre-outbox: move `server.ts`'s HTTP path onto `mintWithOutbox` so a crash mid-composition in the primary demo topology is resumable — DoD: crash-resume test green in serve mode
- [ ] **GAP-12** CI is red and installs no validators: remove the deleted `spike/` step from `.github/workflows/ci.yml`, install pinned typst 0.15.1 + verapdf + poppler-utils — DoD: CI green on push, `npm run verify` passing there
- [ ] **GAP-03** Bursting measurement: run 8,000 docs Typst-only through the real pipeline (fan-out, real render/archive/enqueue), record ms/doc + total wall-clock against the 30-min window in `docs/RESULTS.md` — DoD: numbers in RESULTS.md; **[HUMAN]** then decides ADR-002 on them
...
- [ ] **[HUMAN]** **GAP-06** Print scope: name a site that can't use PDF+OS spooler, or ratify "PDF is the print path"
