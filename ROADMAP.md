...
- [ ] **GAP-15** `rendererVersion` never persisted: `archiveArtifact` writes `rendererId@version` onto the registry row (surfaced by having two renderers — the audit row can no longer say which produced the bytes) — DoD: a pdf-direct row and a Typst row carry different correct values in a test; console's `template@ver · renderer@ver` stops rendering "—"
