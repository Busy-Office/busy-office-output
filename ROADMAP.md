...
- [ ] **GAP-17** Residual per-type switches after GAP-08 (retention years, the payslip authz branch, the console lock glyph — each a `documentType === 'payslip'`-style literal in engine source rather than owner-supplied via `DocumentTypeDefinition`). Low priority: correct today for the three built-ins; about the seam being fully honest — DoD: `DocumentTypeDefinition` optionally carries `retentionYears` + an `ownerIdPath`, the three switches read from the registry, boundary test gains a no-`documentType`-literal rule. *Sequenced after GAP-10 + GAP-16.*
...
### Tasks
- [ ] Template lifecycle draft→review→approved→published→retired; DEV→QAS→PRD transport; author/reviewer/approver/reason recorded — DoD: state machine tests *(arb-chair ruling 2026-08-29, in build: state lives in a persisted append-only `template_lifecycle_log`, the registered meta is declared-initial-state + seed only; **only `published` templates are live candidates** — a real gap, nothing filtered on lifecycle before; **transport collapsed** — `published` = PRD in the one environment that exists (ADR-009), `review` = the QAS analogue, no `environment` column; maintainer confirmed **S1 seeding** (GAP-19) and **separation-of-duties in task 1**; `reason` mandatory on every transition; `Actor` reused unchanged; no new OutputPort verbs)*
