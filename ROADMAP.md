...
an independent gate-check verdict in git history. **Stage 5 does NOT
close today**: ADR-007's OutputPort v1 surface addendum and its v1.1
reprint-verbs amendment are still Proposed, and CLAUDE.md's rule is "no
stage closes with its ADRs open" — see `GATE-S5-CLOSE`. Separately,
GAP-13's ratified exception text says the loop "may proceed into Stage
5"; it does not say Stage 5 may close or Stage 6 may start — flagged,
not assumed, pending the maintainer's read.)*
