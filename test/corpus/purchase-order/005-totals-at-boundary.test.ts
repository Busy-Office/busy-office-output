...
describe('corpus 005-totals-at-boundary', () => {
  ...
  it('pushes the totals block to a fresh page instead of splitting or overflowing it', async () => {
    ...
    // one line fewer (24) fits everything, including totals, on page 1.
    // At 25 lines the totals block no longer fits on page 1 and Typst's
  ...
  it('sanity: one line fewer fits everything on a single page (proves 25 really is the boundary)', async () => {
    const oneLess = generatePurchaseOrder({ ...CORPUS_CASES['005-totals-at-boundary'], lineCount: 24 });
